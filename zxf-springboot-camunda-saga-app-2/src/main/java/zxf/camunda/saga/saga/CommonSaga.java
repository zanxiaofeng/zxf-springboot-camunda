    ...
    public void defineSaga() {
        try {
            if (!sagaRedeploy && isSagaDeployed()) {
                ...
                log.info("{}@app-2 saga had been deployed. (DeploymentId={})", this.sagaName, processDefinition.getDeploymentId());
                return;
            }
            ...
            log.error("Exception when define and deploy {}@app-2 saga", this.sagaName, ex);
        }
    }

    public void trigger(Integer count) {
        Integer times = counter.incrementAndGet();
        log.info("{}@app-2 trigger start, {}::{}", this.sagaName, times, count);
        for (int i = 0; i < count; i++) {
            Map<String, Object> someVariables = new HashMap<>();
            someVariables.put("task-id", this.sagaName + "@app-2@" + times + "::" + i);
            processEngine.getRuntimeService().startProcessInstanceByKey(this.sagaName, someVariables);
        }
        log.info("{}@app-2 trigger end, {}::{}", this.sagaName, times, count);
    }
